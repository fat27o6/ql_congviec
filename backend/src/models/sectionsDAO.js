...
export default class sectionDAO {
    static async injectDB(conn) {
        ...
        try {
            sections = await conn.db(process.env.QLCV_DB_NAME).collection("sections")
            projects = await conn.db(process.env.QLCV_DB_NAME).collection("projects")
        } catch (e) {
    ...
    static async getSectionById(sectionId) {
        try {
            const section = await sections.find({ _id: new ObjectId(sectionId) })
            return section
        } catch (e) {
